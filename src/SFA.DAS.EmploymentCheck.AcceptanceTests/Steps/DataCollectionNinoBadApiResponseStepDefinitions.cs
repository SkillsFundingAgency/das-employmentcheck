...
{
    [Binding]
    [Scope(Feature = "DataCollectionNinoBadApiResponse")]
    public class DataCollectionNinoBadApiResponseStepDefinitions
    {
        ...
        public DataCollectionNinoBadApiResponseStepDefinitions(TestContext context)
        {
            _context = context;
        }
        ...
        public async Task WhenDataCollectionApiCallReturnsStatusCodeAndHas(int statusCode)
        {
            ...
            if (statusCode == 200)
            {
                niNumber = _context.Fixture.Create<string>()[..8];
            }

            _dcApiResponse = new List<LearnerNiNumber>
                { new LearnerNiNumber(_check.Uln, niNumber, httpStatusCode)};
